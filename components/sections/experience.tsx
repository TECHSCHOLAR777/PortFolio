  ...
  return (
    <section id="experience" className="section-y-tight">
      <div className="container-page">
        ...
        <ol className="container-prose border-border/70 mt-10 border-t">
          {experience.map((item) => (
            <li key={`${item.org}-${item.role}`} className="border-border/70 border-b py-5">
              <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
                <h3 className="flex items-center gap-2 font-medium">
                  {item.role}
                  {item.status === 'ongoing' ? (
                    <span className="border-primary/30 bg-primary/10 text-primary rounded-full border px-2 py-0.5 text-xs font-normal">
                      ongoing
                    </span>
                  ) : null}
                </h3>
